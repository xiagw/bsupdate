		...
		if [[ "$latestVersion" != "$currentVersion" ]]; then
			...
			if [[ "$answer" == "Y" || "$answer" == "y" ]]; then
				...
				if [[ -d ~/$repositoryName ]]; then rm -r -f $repositoryName || {
					echo "Error: permissions denied deleting old clone, try running thr update as sudo"
					return 1
				}; fi
				git clone "https://github.com/$githubUserName/$repositoryName" || {
					echo "Couldn't download latest version"
					return 1
				}
				cd $repositoryName || {
					echo 'Update Failed'
				...
				rm -r -f $repositoryName || {
					echo "Error: successfully updated but couldn't delete temperary files at ~/$repositoryName"
					return 1
				}
			else
				return 1
...
checkInternet() {
	httpGet bing.com >/dev/null 2>&1 || {
		echo "Error: no active internet connection" >&2
		return 1
	} # query google with a get request
}
